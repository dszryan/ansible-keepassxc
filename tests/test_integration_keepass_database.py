...
# from ansible_collections.dszryan.keepass.plugins.module_utils.common import EntryDump, Result, RequestQuery
# from ansible_collections.dszryan.keepass.pluginsmodule_utils.database_details import DatabaseDetails
...
#     # def test_ansible_run(self) -> None:
#     #     import subprocess
#     #     cmd = [
#     #         '/usr/bin/ansible-playbook'
#     #         ' -e "machine_name=pegasus machine_username=automata build_ip=192.168.100.244 build_username=root build_password=Q" --vault-password-file /home/rdsouza/OneDrive/Code/github/DytaLabs/Snyp/build/vault.keepass -i /home/rdsouza/OneDrive/Code/github/DytaLabs/Snyp/build/inventory/keepass.yml /home/rdsouza/OneDrive/Code/github/DytaLabs/Snyp/build/build.yml -vvvv']
#     #     process = subprocess.Popen(cmd, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, cwd="/home/rdsouza/OneDrive/Code/github/DytaLabs/Snyp/build")
...
#         actual = KeepassDatabase(DatabaseDetails(self._display, **self._database_details_valid.copy()), None, self._display)._get_binary(base64_encoded)
#         self.assertEqual((message, True), actual)

#     def test__get_binary_plain_text(self):
#         message = "hello world".encode("utf16")
#         actual = KeepassDatabase(DatabaseDetails(self._display, **self._database_details_valid.copy()), None, self._display)._get_binary(message)
#         self.assertEqual((bytes(message), False), actual)

#     def test__open_valid_details(self):
#         storage = KeepassDatabase(DatabaseDetails(self._display, **self._database_details_valid.copy()), None, self._display)
...
#         self.assertRaises(AnsibleParserError, DatabaseDetails, self._display, **self._database_details_invalid_location_invalid.copy())
#         self._display.assert_has_calls([])

#     def test__open_invalid_missing_keyfile(self):
#         database_details = DatabaseDetails(self._display, **self._database_details_invalid_keyfile_missing.copy())
#         self.assertRaises(CredentialsError, KeepassDatabase, database_details, None, self._display)
#         self._display.assert_has_calls([
#             call.vvv("Keepass: database found - %s" % self._database_details_invalid_keyfile_missing["location"])
#         ])
...
#         self.assertRaises(CredentialsError, KeepassDatabase, database_details, None, self._display)
#         self._display.assert_has_calls([
#             call.vvv("Keepass: database found - %s" % self._database_details_invalid_password_missing["location"]),
#             call.vv(u"Keepass: database DEFAULT OPEN - %s" % self._database_details_invalid_password_missing["location"])
#         ])
...
#         database_details = DatabaseDetails(self._display, **database_details_save)
#         storage = KeepassDatabase(database_details, None, self._display)
#         self.assertTrue(isinstance(storage._database, PyKeePass))
#         storage._save()
#         self._display.assert_has_calls([
...
#         database_details = DatabaseDetails(self._display, **self._database_details_valid.copy())
#         storage = KeepassDatabase(database_details, None, self._display)
...
#         database_details = DatabaseDetails(self._display, **database_details_upsert)
#         storage = KeepassDatabase(database_details, None, self._display)
...
#         self._display.assert_has_calls([
#             call.vvv("Keepass: database found - %s" % database_details_upsert["location"]),
...
#         self.assertFalse(has_changed)
#         self.assertEqual(self._database_entry["password"], actual_entry["result"])
#         self._display.assert_has_calls([
#             call.vvv("Keepass: database found - %s" % self._database_details_valid["location"]),
#             call.vvv("Keepass: keyfile found - %s" % self._database_details_valid["keyfile"]),
#             call.vv('Keepass: database DEFAULT OPEN - %s' % self._database_details_valid["location"]),
...
#         self.assertFalse(has_changed)
#         self.assertEqual(self._database_entry["custom_properties"]["test_custom_key"], actual_entry["result"])
#         self._display.assert_has_calls([
#             call.vvv("Keepass: database found - %s" % self._database_details_valid["location"]),
#             call.vvv("Keepass: keyfile found - %s" % self._database_details_valid["keyfile"]),
#             call.vv('Keepass: database DEFAULT OPEN - %s' % self._database_details_valid["location"]),
...
#             self.assertFalse(has_changed)
#             self.assertEqual(base64.b64encode(file.read()), actual_entry["result"])
#         self._display.assert_has_calls([
#             call.vvv("Keepass: database found - %s" % self._database_details_valid["location"]),
#             call.vvv("Keepass: keyfile found - %s" % self._database_details_valid["keyfile"]),
#             call.vv('Keepass: database DEFAULT OPEN - %s' % self._database_details_valid["location"]),
...
#         database_details = DatabaseDetails(self._display, **database_details_delete)
#         storage = KeepassDatabase(database_details, None, self._display)
#         has_changed, deleted_entry = storage.delete(self._delete_entry.query, check_mode=False)
#         self.assertTrue(has_changed)
#         self.assertEqual(None, deleted_entry)
#         self._display.assert_has_calls([
#             call.vvv("Keepass: database found - %s" % database_details_delete["location"]),
